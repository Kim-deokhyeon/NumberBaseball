...
void answerTest({Answer? answerCreateByGameController}) {
  ...
  group('정답은', () {
    ...
    test('중복이 없어야 한다.', () {
      ...
      // 이게 맞는건지?
      expect(answer.isUnique(), false);

      answer = Answer(initAnswer: [3, 3, 3]);

      expect(answer.isUnique(), true);
    });
  });
  ...
  group('특정 숫자의 상태를 판단 할 수 있어야 한다.', () {
    test('정답이 4,5,3 일 경우 볼 검증', () {
      ...
      expect(answer.checkBall(4, 0).runtimeType, bool);
      expect(answer.checkBall(4, 0), false);
      expect(answer.checkBall(4, 1), true);
      expect(answer.checkBall(8, 0), false);
    });

    test('정답이 4,5,3 일 경우 스트라이크 검증', () {
      answer = Answer(initAnswer: [4, 5, 3]);
      expect(answer.checkStrike(4, 0).runtimeType, bool);
      expect(answer.checkStrike(4, 0), true);
      expect(answer.checkStrike(4, 1), false);
    });
  });
}
